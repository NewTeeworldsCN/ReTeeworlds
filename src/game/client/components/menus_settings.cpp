...
void CMenus::RenderSettingsGeneral(CUIRect MainView)
{
	...
	int NumOptions = maximum(Config()->m_ClNameplates ? 6 : 3, 6);
	float ButtonHeight = 20.0f;
	float Spacing = 2.0f;
...
void CMenus::RenderSettingsPlayer(CUIRect MainView)
{
	...
	{
		str_copy(s_aPlayerName, Config()->m_ClPlayerName, sizeof(s_aPlayerName));
	...
	// Personal
	{
		CUIRect Top, Bottom, Name;
		Right.HSplitTop(SpacingH, 0, &Right);
		Right.HSplitMid(&Top, &Bottom, SpacingH);
		Top.HSplitMid(&Name, 0, SpacingH);

		// player name
		Name.HSplitTop(ButtonHeight, &Button, &Name);
		static CLineInput s_NameInput(Config()->m_ClPlayerName, sizeof(Config()->m_ClPlayerName), MAX_NAME_LENGTH);
	...
	m_NeedRestartPlayer = !(str_comp(s_aPlayerName, Config()->m_ClPlayerName)
	);
...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	// reset warning
	bool NeedReconnect = m_NeedRestartPlayer;

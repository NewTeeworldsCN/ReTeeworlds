...
void CMenus::RenderLoading(int WorkedAmount)
{
	...
	TextRender()->TextColor(CUI::ms_DefaultTextColor);
	TextRender()->TextSecondaryColor(CUI::ms_DefaultTextOutlineColor);
	UI()->DoLabel(&Rect, "ReTeeworlds", 48.0f, TEXTALIGN_CENTER);
...
void CMenus::RenderMenu(CUIRect Screen)
{
	...
	if(m_Popup == POPUP_NONE)
	{
		...
		else
		{
			...
			// render top buttons
			{
				...
				// settings button
				if(Client()->State() == IClient::STATE_OFFLINE && m_MenuPage == PAGE_DEMOS)
				{
	...
	else
	{
		...
		else if(m_Popup == POPUP_PASSWORD)
		{
			...
			{
				Client()->Connect(m_aPasswordPopupServerAddress);
				m_aPasswordPopupServerAddress[0] = '\0';
			}
		}
		else if(m_Popup == POPUP_CONNECTING)
		...
		else if(m_Popup == POPUP_FIRST_LAUNCH)
		{
			...
			static CLineInput s_PlayerNameInput(Config()->m_ClPlayerName, sizeof(Config()->m_ClPlayerName), MAX_NAME_LENGTH);

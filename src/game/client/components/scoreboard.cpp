...
float CScoreboard::RenderScoreboard(float x, float y, float w, int Team, const char *pTitle, int Align)
{
	...
	float TeeOffset = 4.0f, TeeLength = 25*TeeSizeMod;
	float NameOffset = IdSize, NameLength = 128.0f-IdSize/2-ReadyLength;
	...
	const char *pScoreStr = Localize("Score");
	TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);

	s_Cursor.Reset();
	s_Cursor.m_Align = TEXTALIGN_CENTER;
	s_Cursor.MoveTo(ScoreOffset + ScoreLength / 2, y + Spacing);
	...
	for(int i = 0 ; i < NumRenderScoreIDs ; i++)
	{
		if(RenderScoreIDs[i] >= 0)
		{
			...
			if(!(m_pClient->m_LocalClientID == pInfo->m_ClientID && Config()->m_ClHideSelfScore))
			{
				// K
				TextRender()->TextColor(TextColor.r, TextColor.g, TextColor.b, 0.5f*ColorAlpha);
...
void CScoreboard::OnRender()
{
	...
	const char* pRedClanName = Localize("Red team");
	const char* pBlueClanName = Localize("Blue team");
